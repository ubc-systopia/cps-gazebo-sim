...
def generate_launch_description():
    ...
    moveit_cfg = (
        MoveItConfigsBuilder("diff_robots", package_name='diff_robots')
        .to_moveit_configs()
    )
    ...
    robot_xacro_cmd = Command([
        ...
        PathJoinSubstitution([FindPackageShare('diff_robots'), 'urdf', 'diff_robots.urdf.xacro'])
    ])
    ...
    rviz_node = Node (
        ...
        arguments=["-d", os.path.join(
            get_package_share_directory("diff_robots"),
            "config", "diff_robots.rviz"
        )],
        parameters=[
    ...
    pkg_share = get_package_share_directory('diff_robots')
