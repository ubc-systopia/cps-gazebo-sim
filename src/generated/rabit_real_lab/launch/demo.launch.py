...
def generate_launch_description():
    ...
    moveit_cfg = (
        MoveItConfigsBuilder("rabit_real_lab", package_name='rabit_real_lab')
        .to_moveit_configs()
    )
    ...
    robot_xacro_cmd = Command([
        ...
        PathJoinSubstitution([FindPackageShare('rabit_real_lab'), 'urdf', 'rabit_real_lab.urdf.xacro'])
    ])
    ...
    _world = os.path.join(get_package_share_directory("rabit_real_lab"), "config", "world.sdf")
    ...
    rviz_node = Node (
        ...
        arguments=["-d", os.path.join(
            get_package_share_directory("rabit_real_lab"),
            "config", "rabit_real_lab.rviz"
        )],
        parameters=[
    ...
    pkg_share = get_package_share_directory('rabit_real_lab')
