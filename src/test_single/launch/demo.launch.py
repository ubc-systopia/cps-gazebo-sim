...
def generate_launch_description():
    ...
    moveit_cfg = (
        MoveItConfigsBuilder("test_single", package_name='test_single')
        .to_moveit_configs()
    )
    ...
    robot_xacro_cmd = Command([
        ...
        PathJoinSubstitution([FindPackageShare('test_single'), 'urdf', 'test_single.urdf.xacro'])
    ])
    ...
    rviz_node = Node (
        ...
        arguments=["-d", os.path.join(
            get_package_share_directory("test_single"),
            "config", "test_single.rviz"
        )],
        parameters=[
    ...
    pkg_share = get_package_share_directory('test_single')
