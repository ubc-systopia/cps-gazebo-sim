...
class RealisticCollisionTester(Node):
    ...
    async def check_collision(self, arm1_positions, arm2_positions, description=""):
        ...
        try:
            ...
            if response is None:
                ...
                return True, 0, 0  # Assume collision for safety
            ...
            if not response.valid:
                ...
                # Show details for inter-arm collisions
    ...
    async def run_realistic_collision_tests(self):
        ...
        self.get_logger().info('🎯 Starting REALISTIC collision detection tests...')
        ...
        for i, scenario in enumerate(scenarios, 1):
            ...
            if has_collision:
                ...
                if inter_arm > 0:
                    ...
                    self.get_logger().warn(f'   ⚠️  RESULT: Intra-arm collision only')
            else:
        ...
        # Final summary
        self.get_logger().info('📊 REALISTIC COLLISION TEST SUMMARY')
        ...
        if total_inter_arm > 0:
            ...
            self.get_logger().warn('⚠️  ANALYSIS: No inter-arm collisions detected')
            self.get_logger().warn('   This could mean:')
            self.get_logger().warn('   1. Inter-arm collisions are disabled in SRDF')
            self.get_logger().warn('   2. Arms are positioned too far apart')
            self.get_logger().warn('   3. Test scenarios need adjustment')
        
        if total_intra_arm > 0:
            self.get_logger().info(f'ℹ️  Note: {total_intra_arm} intra-arm collisions detected (expected for some poses)')
